// Built from  inst/include/plant/models/ff16_strategy.h on Wed Jul 22 20:33:17 2020 using the scaffolder, from the strategy:  FF16
// -*-c++-*-
#ifndef PLANT_PLANT_ST_00_STRATEGY_H_
#define PLANT_PLANT_ST_00_STRATEGY_H_
...
#include <plant/models/st_00_environment.h>
#include <plant/models/assimilation.h>

namespace plant {

class ST_00_Strategy: public Strategy<ST_00_Environment> {
public:
  typedef std::shared_ptr<ST_00_Strategy> ptr;
  ST_00_Strategy();
  ...
  // Translate generic methods to ST_00 strategy leaf area methods
  ...
  void compute_rates(const ST_00_Environment& environment, bool reuse_intervals,
                Internals& vars);
  ...
  double net_mass_production_dt(const ST_00_Environment& environment,
  ...
  double establishment_probability(const ST_00_Environment& environment);
  ...
  // Set constants within ST_00_Strategy
  void prepare_strategy();

  // Previously there was an "integrator" here.  I'm going to stick
  // that into Control or ST_00_Environment instead.
 ...
 // Baseline for growth mortality rate
  ...
  Assimilation<ST_00_Environment> assimilator;

  
  // make polymorphic
  virtual ~ST_00_Strategy() {}
