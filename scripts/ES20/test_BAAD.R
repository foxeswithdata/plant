...
p1_es <- expand_parameters(trait_matrix(c(0.60, 21.9, 0.2), c("t_s", "a_s", "height_0")), p0_es)
...
linearMod <- lm((m.rf) ~ a.lf, data=euc_data)
...
linearMod <- lm((a.ssbh) ~ a.lf, data=euc_data)
...
linearMod <- lm((a.sbbh) ~ a.ssbh, data=euc_data)

summary(linearMod)

p <- ggplot(euc_data, aes(x=a.ssbh, shape=species)) +  
  geom_point(alpha=0.25, aes(y=a.sbbh), color = "blue") +
  scale_x_continuous(name = "Bark Area (m2)") +
  scale_y_continuous(name = "Sapwood area (m2)") + 
  theme_linedraw()
p
