// Built from  src/ff16_strategy.cpp on Wed Jul 22 20:33:17 2020 using the scaffolder, from the strategy:  FF16
...
void ST_00_Strategy::refresh_indices () {
...
double ST_00_Strategy::area_leaf(double height) const {
  return pow(height / a_l1, 1.0 / a_l2);
}

// [eqn 1] mass_leaf (inverse of [eqn 2])
double ST_00_Strategy::mass_leaf(double area_leaf) const {
  return area_leaf * lma;
}

// [eqn 4] area and mass of sapwood
double ST_00_Strategy::area_sapwood(double area_leaf) const {
  return area_leaf * theta;
}

double ST_00_Strategy::mass_sapwood(double area_sapwood, double height) const {
  return area_sapwood * height * eta_c * rho;
}

// [eqn 5] area and mass of bark
double ST_00_Strategy::area_bark(double area_leaf) const {
  return a_b1 * area_leaf * theta;
}

double ST_00_Strategy::mass_bark(double area_bark, double height) const {
  return area_bark * height * eta_c * rho;
}

double ST_00_Strategy::area_stem(double area_bark, double area_sapwood,
                            double area_heartwood) const {
  return area_bark + area_sapwood + area_heartwood;
}

double ST_00_Strategy::diameter_stem(double area_stem) const {
  return std::sqrt(4 * area_stem / M_PI);
}

// [eqn 7] Mass of (fine) roots
double ST_00_Strategy::mass_root(double area_leaf) const {
  return a_r1 * area_leaf;
}

// [eqn 8] Total mass
double ST_00_Strategy::mass_live(double mass_leaf, double mass_bark,
                           double mass_sapwood, double mass_root) const {
  return mass_leaf + mass_sapwood + mass_bark + mass_root;
}

double ST_00_Strategy::mass_total(double mass_leaf, double mass_bark,
...
double ST_00_Strategy::mass_above_ground(double mass_leaf, double mass_bark,
                            double mass_sapwood, double mass_root) const {
  return mass_leaf + mass_bark + mass_sapwood + mass_root;
}

// for updating auxillary state
void ST_00_Strategy::update_dependent_aux(const int index, Internals& vars) {
...
void ST_00_Strategy::compute_rates(const ST_00_Environment& environment,
...
double ST_00_Strategy::respiration(double mass_leaf, double mass_sapwood,
...
double ST_00_Strategy::respiration_leaf(double mass) const {
  return r_l * mass;
}

double ST_00_Strategy::respiration_bark(double mass) const {
  return r_b * mass;
}

double ST_00_Strategy::respiration_sapwood(double mass) const {
  return r_s * mass;
}

double ST_00_Strategy::respiration_root(double mass) const {
  return r_r * mass;
}

// [eqn 14] Total turnover
double ST_00_Strategy::turnover(double mass_leaf, double mass_bark,
                          double mass_sapwood, double mass_root) const {
...
double ST_00_Strategy::turnover_leaf(double mass) const {
  return k_l * mass;
}

double ST_00_Strategy::turnover_bark(double mass) const {
  return k_b * mass;
}

double ST_00_Strategy::turnover_sapwood(double mass) const {
  return k_s * mass;
}

double ST_00_Strategy::turnover_root(double mass) const {
  return k_r * mass;
}
...
double ST_00_Strategy::net_mass_production_dt_A(double assimilation, double respiration,
...
double ST_00_Strategy::net_mass_production_dt(const ST_00_Environment& environment,
...
double ST_00_Strategy::fraction_allocation_reproduction(double height) const {
  return a_f1 / (1.0 + exp(a_f2 * (1.0 - height / hmat)));
}

// Fraction of production allocated to growth
double ST_00_Strategy::fraction_allocation_growth(double height) const {
  return 1.0 - fraction_allocation_reproduction(height);
}

// [eqn 17] Rate of offspring production
double ST_00_Strategy::fecundity_dt(double net_mass_production_dt,
                               double fraction_allocation_reproduction) const {
  return net_mass_production_dt * fraction_allocation_reproduction /
    (omega + a_f3);
}

double ST_00_Strategy::darea_leaf_dmass_live(double area_leaf) const {
...
double ST_00_Strategy::area_heartwood_dt(double area_leaf) const {
  return k_s * area_sapwood(area_leaf);
}

// Growth rate of bark area at base per unit time
double ST_00_Strategy::area_bark_dt(double area_leaf_dt) const {
  return a_b1 * area_leaf_dt * theta;
}

// Growth rate of stem basal area per unit time
double ST_00_Strategy::area_stem_dt(double area_leaf,
...
// Growth rate of basal diameter_stem per unit time
double ST_00_Strategy::diameter_stem_dt(double area_stem, double area_stem_dt) const {
  return ddiameter_stem_darea_stem(area_stem) * area_stem_dt;
}

// Growth rate of root mass per unit time
double ST_00_Strategy::mass_root_dt(double area_leaf,
                               double area_leaf_dt) const {
  return area_leaf_dt * dmass_root_darea_leaf(area_leaf);
}

double ST_00_Strategy::mass_live_dt(double fraction_allocation_reproduction,
...
double ST_00_Strategy::mass_total_dt(double fraction_allocation_reproduction,
...
double ST_00_Strategy::mass_above_ground_dt(double area_leaf,
...
double ST_00_Strategy::mass_heartwood_dt(double mass_sapwood) const {
  return turnover_sapwood(mass_sapwood);
}


double ST_00_Strategy::mass_live_given_height(double height) const {
...
double ST_00_Strategy::height_given_mass_leaf(double mass_leaf) const {
  return a_l1 * pow(mass_leaf / lma, a_l2);
}

double ST_00_Strategy::mortality_dt(double productivity_area,
...
double ST_00_Strategy::mortality_growth_independent_dt() const {
  return d_I;
}

double ST_00_Strategy::mortality_growth_dependent_dt(double productivity_area) const {
  return a_dG1 * exp(-a_dG2 * productivity_area);
}

// [eqn 20] Survival of seedlings during establishment
double ST_00_Strategy::establishment_probability(const ST_00_Environment& environment) {
  const double net_mass_production_dt_ =
...
double ST_00_Strategy::area_leaf_above(double z, double height) const {
  return area_leaf(height) * Q(z, height);
}

// [eqn 10] ... Fraction of leaf area above height 'z' for an
//              individual of height 'height'
double ST_00_Strategy::Q(double z, double height) const {
  if (z > height) {
    return 0.0;
  }
...
double ST_00_Strategy::height_seed(void) const {
...
void ST_00_Strategy::prepare_strategy() {
...
ST_00_Strategy::ptr make_strategy_ptr(ST_00_Strategy s) {
  s.prepare_strategy();
  return std::make_shared<ST_00_Strategy>(s);
}
}
