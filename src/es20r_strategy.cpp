...
#include <plant/models/es20r_environment.h>
...
ES20r_Strategy::ES20r_Strategy() {
  using std::cout;
  using std::cerr;
  using std::endl;
  cout << "ES20r strategy constructor" << endl;
...
void ES20r_Strategy::refresh_indices () {
...
double ES20r_Strategy::mass_leaf(double area_leaf) const {
  return area_leaf * lma;
}

// [eqn 4] area and mass of sapwood
double ES20r_Strategy::area_sapwood(double area_leaf) const {
  return area_leaf * theta;
}

double ES20r_Strategy::mass_sapwood(double area_sapwood, double height) const {
  return area_sapwood * height * eta_c * rho;
}

// [eqn 5] area and mass of bark
double ES20r_Strategy::area_bark(double area_leaf) const {
  return a_b1 * area_leaf * theta;
}

double ES20r_Strategy::mass_bark(double area_bark, double height) const {
  return area_bark * height * eta_c * rho;
}

double ES20r_Strategy::area_stem(double area_bark, double area_sapwood,
                                double area_heartwood) const {
  return area_bark + area_sapwood + area_heartwood;
}

double ES20r_Strategy::diameter_stem(double area_stem) const {
  return std::sqrt(4 * area_stem / M_PI);
}

// [eqn 7] Mass of (fine) roots
double ES20r_Strategy::mass_root(double area_leaf) const {
  return a_r1 * area_leaf;
}

// [eqn 8] Total mass
double ES20r_Strategy::mass_live(double mass_leaf, double mass_bark,
                                double mass_sapwood, double mass_root) const {
  return mass_leaf + mass_sapwood + mass_bark + mass_root;
}

double ES20r_Strategy::mass_total(double mass_leaf, double mass_bark,
...
double ES20r_Strategy::mass_above_ground(double mass_leaf, double mass_bark,
                                        double mass_sapwood, double mass_root) const {
  return mass_leaf + mass_bark + mass_sapwood + mass_root;
}

// for updating auxillary state
void ES20r_Strategy::update_dependent_aux(const int index, Internals& vars) {
...
double ES20r_Strategy::dbiomass_dt(const ES20r_Environment& environment, 
...
double ES20r_Strategy::dmass_storage_dt(double net_mass_production_dt_, double dbiomass_dt_) const {
...
void ES20r_Strategy::compute_rates(const ES20r_Environment& environment,
...
double ES20r_Strategy::respiration(double mass_leaf, double mass_sapwood,
...
double ES20r_Strategy::respiration_leaf(double mass) const {
  return r_l * mass;
}

double ES20r_Strategy::respiration_bark(double mass) const {
  return r_b * mass;
}

double ES20r_Strategy::respiration_sapwood(double mass) const {
  return r_s * mass;
}

double ES20r_Strategy::respiration_root(double mass) const {
  return r_r * mass;
}

// [eqn 14] Total turnover
double ES20r_Strategy::turnover(double mass_leaf, double mass_bark,
                               double mass_sapwood, double mass_root) const {
...
double ES20r_Strategy::turnover_leaf(double mass) const {
  return k_l * mass;
}

double ES20r_Strategy::turnover_bark(double mass) const {
  return k_b * mass;
}

double ES20r_Strategy::turnover_sapwood(double mass) const {
  return k_s * mass;
}

double ES20r_Strategy::turnover_root(double mass) const {
  return k_r * mass;
}
...
double ES20r_Strategy::net_mass_production_dt_A(double assimilation, double respiration) const {
...
double ES20r_Strategy::net_mass_production_dt(const ES20r_Environment& environment,
...
double ES20r_Strategy::area_leaf_dt(double darea_leaf_dmass_live_, double dbiomass_dt_) const{
  return darea_leaf_dmass_live_ * dbiomass_dt_;
}

double ES20r_Strategy::mass_leaf_dt(double area_leaf, double darea_leaf_dt_) const {
  return dmass_leaf_darea_leaf(area_leaf) * darea_leaf_dt_ ;
}

// [eqn 2] area_leaf (inverse of [eqn 3])
double ES20r_Strategy::area_leaf(double height) const {
  return pow(height / a_l1, 1.0 / a_l2);
}

// [eqn 17] Rate of offspring production
double ES20r_Strategy::fecundity_dt(double mass_storage, double height) const {
  return mass_storage * a_f1 / ((omega + a_f3) * (1.0 + exp(a_f2 * (1.0 - height / hmat))));
}

double ES20r_Strategy::darea_leaf_dmass_live(double area_leaf) const {
...
double ES20r_Strategy::dheight_darea_leaf(double area_leaf) const {
  return a_l1 * a_l2 * pow(area_leaf, a_l2 - 1);
}

// Mass of leaf needed for new unit area leaf, d m_s / d a_l
double ES20r_Strategy::dmass_leaf_darea_leaf(double /* area_leaf */) const {
  return lma;
}

// Mass of stem needed for new unit area leaf, d m_s / d a_l
double ES20r_Strategy::dmass_sapwood_darea_leaf(double area_leaf) const {
  return rho * eta_c * a_l1 * theta * (a_l2 + 1.0) * pow(area_leaf, a_l2);
}

// Mass of bark needed for new unit area leaf, d m_b / d a_l
double ES20r_Strategy::dmass_bark_darea_leaf(double area_leaf) const {
  return a_b1 * dmass_sapwood_darea_leaf(area_leaf);
}

// Mass of root needed for new unit area leaf, d m_r / d a_l
double ES20r_Strategy::dmass_root_darea_leaf() const {
  return a_r1;
}

// Growth rate of basal diameter_stem per unit time
double ES20r_Strategy::ddiameter_stem_darea_stem(double area_stem) const {
  return pow(M_PI * area_stem, -0.5);
}

// Growth rate of sapwood area at base per unit time
double ES20r_Strategy::area_sapwood_dt(double area_leaf_dt) const {
  return area_leaf_dt * theta;
}

double ES20r_Strategy::mass_sapwood_dt(double dleaf_area_dt_, double dmass_sapwood_darea_leaf_) const{
...
double ES20r_Strategy::area_heartwood_dt(double area_sapwood) const {
  return k_s * area_sapwood;
}

// Growth rate of bark area at base per unit time
double ES20r_Strategy::area_bark_dt(double area_leaf_dt) const {
  return a_b1 * area_leaf_dt * theta;
}

// Growth rate of bark mass at base per unit time
double ES20r_Strategy::mass_bark_dt(double dsapwood_mass_dt) const {
  return a_b1 * dsapwood_mass_dt;
}

// Growth rate of stem basal area per unit time
double ES20r_Strategy::area_stem_dt(double area_leaf,
...
// Growth rate of basal diameter_stem per unit time
double ES20r_Strategy::diameter_stem_dt(double area_stem, double area_stem_dt) const {
  return ddiameter_stem_darea_stem(area_stem) * area_stem_dt;
}

// Growth rate of root mass per unit time
double ES20r_Strategy::mass_root_dt(double area_leaf_dt) const {
  return area_leaf_dt * dmass_root_darea_leaf();
}

double ES20r_Strategy::mass_live_dt(double fraction_allocation_reproduction,
...
double ES20r_Strategy::mass_total_dt(double fraction_allocation_reproduction,
...
double ES20r_Strategy::mass_above_ground_dt(double area_leaf,
...
double ES20r_Strategy::mass_heartwood_dt(double mass_sapwood) const {
  return turnover_sapwood(mass_sapwood);
}


double ES20r_Strategy::height_given_mass_leaf(double mass_leaf) const {
  return a_l1 * pow(mass_leaf / lma, a_l2);
}

double ES20r_Strategy::mortality_dt(double storage_portion,
...
double ES20r_Strategy::mortality_growth_independent_dt() const {
  return d_I;
}

double ES20r_Strategy::mortality_growth_dependent_dt(double storage_portion) const {
  return a_dG1 * exp(-a_dG2 * storage_portion);
}

// [eqn 20] Survival of seedlings during establishment
double ES20r_Strategy::establishment_probability(const ES20r_Environment& environment) {
...
double ES20r_Strategy::area_leaf_above(double z, double height) const {
  return area_leaf(height) * Q(z, height);
}

// [eqn 10] ... Fraction of leaf area above height 'z' for an
//              individual of height 'height'
double ES20r_Strategy::Q(double z, double height) const {
  if (z > height) {
    return 0.0;
  }
...
double ES20r_Strategy::mass_live_given_height(double height) const {
...
double ES20r_Strategy::height_seed(void) const {
...
void ES20r_Strategy::prepare_strategy() {
...
ES20r_Strategy::ptr make_strategy_ptr(ES20r_Strategy s) {
  s.prepare_strategy();
  return std::make_shared<ES20r_Strategy>(s);
}
}
