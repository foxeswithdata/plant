# Built from  tests/testthat/test-strategy-ff16.R on Wed Sep 16 07:11:11 2020 using the scaffolder, from the strategy:  FF16
context("Strategy-ES20")
...
test_that("ES20 collect_all_auxillary option", {
  ...
  s <- ES20_Strategy(collect_all_auxillary=TRUE)
...
test_that("Critical Names", {
  s <- ES20_Strategy()
  my_names <- ES20_Individual(s)$ode_names
  expect_identical(my_names[1:3], c("height", "mortality", "fecundity"))
})
test_that("ES20_Strategy hyper-parameterisation", {
  s <- ES20_Strategy()

  # lma
  lma <- c(0.1,1)
  ret <- ES20_hyperpar(trait_matrix(lma, "lma"), s)
  ...
  # wood density
  rho <- c(200,300)
  ret <- ES20_hyperpar(trait_matrix(rho, "rho"), s)
  ...
  ret <- ES20_hyperpar(trait_matrix(narea, "narea"), s)
  ...
  ret <- ES20_hyperpar(trait_matrix(omega, "omega"), s)
  ...
  ## Empty trait matrix:
  ret <- ES20_hyperpar(trait_matrix(numeric(0), "lma"), s)
...
test_that("narea calculation", {
  ...
  expect_silent(sl <- strategy_list(m, p0, ES20_hyperpar))

  cmp <- lapply(x, function(xi) strategy(trait_matrix(xi, "hmat"), p0, ES20_hyperpar))
  expect_equal(sl, cmp)
})
...
test_that("seed rain", {
  ...
  # one species
  p1 <- expand_parameters(trait_matrix(0.0825, "lma"), p0, ES20_hyperpar,FALSE)
  ...
  # two species
  p2 <- expand_parameters(trait_matrix(0.2625, "lma"), p1, ES20_hyperpar, FALSE)
