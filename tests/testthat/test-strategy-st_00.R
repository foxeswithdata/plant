# Built from  tests/testthat/test-strategy-ff16.R on Wed Jul 22 20:33:17 2020 using the scaffolder, from the strategy:  FF16
context("Strategy-ST_00")
...
test_that("ST_00 collect_all_auxillary option", {
  ...
  s <- ST_00_Strategy(collect_all_auxillary=TRUE)
...
test_that("Critical Names", {
  s <- ST_00_Strategy()
  my_names <- ST_00_Plant(s)$ode_names
  expect_identical(my_names[1:3], c("height", "mortality", "fecundity"))
})
test_that("ST_00_Strategy hyper-parameterisation", {
  s <- ST_00_Strategy()

  # lma
  lma <- c(0.1,1)
  ret <- ST_00_hyperpar(trait_matrix(lma, "lma"), s)
  ...
  # wood density
  rho <- c(200,300)
  ret <- ST_00_hyperpar(trait_matrix(rho, "rho"), s)
  ...
  ret <- ST_00_hyperpar(trait_matrix(narea, "narea"), s)
  ...
  ret <- ST_00_hyperpar(trait_matrix(omega, "omega"), s)
  ...
  ## Empty trait matrix:
  ret <- ST_00_hyperpar(trait_matrix(numeric(0), "lma"), s)
